function [du] = BurgersFLrhs1D(x, u, h, k, maxvel)
    % function [du] = BurgersFLrhs1D(x, u, h, k, maxvel)
    % Purpose: Evaluate right hand side for Burgers equation using a flux
    % limited scheme
    N = length(x);
    
    % Choose flux limiter - 0:LF; 1:CO; 2:Koren; 3:Sweby; 4:OSPRE; 5:van
    % Leer
    type = 5;
    beta = 2;
    % Boundary Conditions
    [xe, ue] = extend(x, u, h, 1, 'P', 0, 'P', 0); % Periodic boundary conditions
    % [xe, ue] = extend(x, u, h, 1, 'D', 2, 'P', 0); % Consistent boundary conditions
   ...
   phiLm = FluxLimit(re(2:N+1), type, beta);
   phiRm = FluxLimit(re(3:N+2), type, beta);
   ...
   Fluxlow = BurgersLF(ue(1:N),ue(2:N+1),0,macvel);
